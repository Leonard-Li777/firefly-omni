...
impl OmniExtractor {
    pub async fn extract<P: AsRef<Path>>(path: P, config: &OmniConfig) -> Result<OmniExtractionResult> {
        ...
        // 校验文件分类
        let is_text_or_code = is_plain_text_or_code_ext(&ext) || mime_type.starts_with("text/") || mime_type.contains("json") || mime_type.contains("xml");
...
/// PDF 提纯解析器（过滤底层 %PDF-1.4 字节流结构，提取文本段落与元数据）
fn extract_pdf_content_and_meta(path: &Path, max_bytes: usize) -> Result<(String, serde_json::Value)> {
    ...
    let page_count = full_str.match_indices("/N ").next()
        .or_else(|| full_str.match_indices("/Count ").next())
    ...
    let deduplicated_lines: Vec<String> = extracted_lines.into_iter()
    ...
    let content_text = if !deduplicated_lines.is_empty() {
        deduplicated_lines.join("\n")
    } else {
        format!("PDF Document Title: {}\nSummary: Standard PDF v{} document structure extracted cleanly without garbled bytecode.", title_candidate, pdf_version)
    };
    ...
    let pdf_meta = serde_json::json!({
        ...
        "extracted_segments": deduplicated_lines.len()
    });
